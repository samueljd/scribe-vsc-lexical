...
export function getLookupStringsForBook(book: string) {
  ...
  const lookupStrings = [bookData.name, bookData.abbr];
...
export function validateVrefAgainstORG(vref: string) {
  ...
  if (!vrefData[book].chapters[chapter]) {
    return false;
  }
  if (vrefData[book].chapters[chapter] < parseInt(verse)) {
    return false;
  }
  return true;
}
